...
The checker consumes plan JSON from standard input so sensitive plan values are
not written to disk. Never write or publish raw plan JSON, and never put the
Google OAuth value in a variable file. The root reads the environment-specific
secret directly from AWS Secrets Manager. OpenTofu owns the secret's name,
tags, recovery behavior, and deletion lifecycle; an operator or rotation
process owns its value.
